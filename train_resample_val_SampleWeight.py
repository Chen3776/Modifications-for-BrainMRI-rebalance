    ...
    for i in range(labels.shape[0]):
        ...
        for c in range(len(Pc_list)):
            if labels[i,c] in [0, 1]:
                score += Pc_list[c] if labels[i,c] == 1 else Nc_list[c]
                score_num += 1
...
def train(model, image_encoder, text_encoder, fuseModule, tokenizer, datasets, Pc_list, Nc_list, optimizer, epoch, warmup_steps, device, scheduler, config):
    ...
    fid_list = np.load(config['fid_list_file'])
    ...
    # weights: len 2972 list, sample wise weight
    score_list , weights = get_weights(label_list,Pc_list,Nc_list)
    ...
    for i, sample in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        fids = sample['fid'] # len 16 list, corresponding to weights(list len 2972)
        ...
        # now have fid list
        # fid list -> fid index
        # fid index -> weight list
        
        fids_index = np.where(np.isin(np.array(fid_list), fids))[0]
        fids_index = []
        for fid_ in fids:
            for index_ in range(len(fid_list)):
                if fid_ == fid_list[index_]:
                    fids_index.append(index_)
                    break

        # print(len(fids_index))

        weight = [weights[index] for index in fids_index] * 13
        weight = torch.tensor(weight).to(device)
        weight = weight[Mask]
        # print(weight)

        loss_ce = F.binary_cross_entropy(logits[:,0],labels[:,0], weight=weight)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='/remote-home/pengyichen/baseline_1/CODE/Pretrain/output_dir/output_resample_SampleWeight')
